...
const allEmployees = async (req, res) => {
    ...
    try {
        ...
        const count = await db.execute((0, drizzle_orm_1.sql) `select count(${employeesTable.employeeID}) from ${employeesTable}`);
        const countInfo = {
            queryString: `select count(1) from employees`,
            queryTS: countTS,
        ...
        const queryInfo = {
            queryString: db.select(employeesTable).limit(limit).offset(offset).toSQL()
                .sql,
            queryTS,
