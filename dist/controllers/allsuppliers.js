...
const allSuppliers = async (req, res) => {
    ...
    try {
        ...
        const count = await db.execute((0, drizzle_orm_1.sql) `select count(${suppliersTable.supplierID}) from ${suppliersTable}`);
        const countInfo = {
            queryString: `select count(1) from suppliers`,
            queryTS: countTS,
        ...
        const query = await db.select(suppliersTable).limit(limit).offset(offset);
        const queryInfo = {
            queryString: db.select(suppliersTable).limit(limit).offset(offset).toSQL()
                .sql,
            queryTS,
